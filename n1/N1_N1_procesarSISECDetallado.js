/**
 * Procesa los movimientos laborales de cada patrón detectado.
 * Nivel N1
 */
function N1_procesarSISECDetallado(patronActual, movimientos, fechaBajaGeneral, vigente) {
  ...
  for (let i = 0; i < movimientosValidos.length; i++) {
    let fechaAlta =
      i === 0 ? movimientosValidos[i].fecha : N1_restarUnDia(movimientosValidos[i - 1].fecha);
    let salario = movimientosValidos[i].salario;
    let fechaBaja =
      i === 0 ? fechaBajaFinal : N1_restarUnDia(movimientosValidos[i - 1].fecha);
